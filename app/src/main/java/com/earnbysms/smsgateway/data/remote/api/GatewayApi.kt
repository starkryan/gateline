...
interface GatewayApi {
    ...
    suspend fun registerDevice(@Body deviceInfo: DeviceInfo): DeviceRegistrationResponse

    /**
     * Forward SMS message to GOIP server
     */
    @POST("api/sms/receive")
    suspend fun forwardSms(@Body smsMessage: SmsMessage): SmsForwardingResponse

    /**
     * Send device heartbeat
     */
    @POST("api/device/heartbeat")
    suspend fun sendHeartbeat(@Body heartbeatData: HeartbeatData): HeartbeatResponse
}
